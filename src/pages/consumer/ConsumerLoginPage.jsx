...
export default function ConsumerLoginPage() {
  ...
  async function handleSubmit() {
    ...
    const result = await login('consumer', form.email, form.password);
    setLoading(false);
      ...
      <div className="relative grid gap-10 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">
        <div>
          <div className="inline-flex rounded-full border border-emerald-400/20 bg-emerald-400/10 px-4 py-2 text-sm text-emerald-300">
            Area do consumidor final
          </div>

          <h1 className="mt-6 max-w-3xl text-4xl font-black leading-tight md:text-6xl">
            Entre para acompanhar suas <span className="text-emerald-400">ofertas favoritas</span>.
          </h1>
          ...
          <div className="mt-10 grid gap-4 sm:grid-cols-3">
            <MetricCard label="Acesso" value="Consumidor" icon={User} tone="emerald" />
            <MetricCard label="Ofertas" value="Vitrine ativa" icon={ShoppingBag} tone="blue" />
            <MetricCard label="Conta" value="Login rapido" icon={CheckCircle2} tone="amber" />
          </div>
        </div>

        <div className="rounded-[32px] border border-white/10 bg-neutral-900/90 p-6 shadow-2xl shadow-black/30 backdrop-blur md:p-8">
          <div className="mb-6">
            <div className="text-2xl font-black">Entrar como consumidor</div>
            <p className="mt-2 text-white/62">Acesse seu historico de pedidos e as ofertas disponiveis.</p>
          </div>
          ...
          <div className="mt-5 flex flex-col gap-2 text-sm">
            <Link to="/consumer/register" className="text-emerald-300 hover:text-emerald-200">
